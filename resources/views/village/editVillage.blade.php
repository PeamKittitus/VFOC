...
<div class="card card-info">
    <div class="card-header">
        <h3 class="card-title">แก้ไขยื่นคำขอขึ้นทะเบียน</h3>
    </div>
    ...
    <form id="editVillage" name="editVillage" method="post" enctype="multipart/form-data">
        <div class="box-body">
            <div class="form-group">
                ...
                <input type="text" class="form-control checkNumber" id="VillageCodeText" maxlength="8" value="{{$getVillageById->VillageCodeText}}">
            </div>
            <div class="form-group">
                <label>รหัสผู้เสียภาษี/เลขนิติบุคคล ( 13 หลัก) <span style="color:red">*</span></label>
                <input type="text" class="form-control checkNumber" id="VillageDbd" maxlength="13" value="{{$getVillageById->VillageDbd}}">
            </div>
            <div class="form-group">
                <label>เลขทะเบียนใบนิติบุคคล <span style="color:red">*</span></label>
                <input type="text" class="form-control checkNumber" id="VillageBdbCode" value="{{$getVillageById->VillageBdbCode}}">
            </div>
            <div class="form-group">
                <label>ชื่อนิติบุคคล <span style="color:red">*</span></label>
                <input type="text" class="form-control" id="VillageName" value="{{$getVillageById->VillageName}}">
            </div>
            <div class="form-group">
                <label>เบอร์โทรศัพท์<span style="color:red">*</span></label>
                <input type="text" class="form-control checkNumber" id="Phone" maxlength="10" value="{{$getVillageById->Phone}}">
            </div>
            <div class="form-group">
                <label>อีเมล (e-mail)</label>
                <input type="text" class="form-control" id="Email" value="{{$getVillageById->Email}}">
            </div>
            <div class="form-group">
                <label>วันที่จัดตั้ง<span style="color:red">*</span></label>
                <input type="date" class="form-control" id="DbdDate" value="{{$getVillageById->DbdDate}}">
            </div>
            <div class="form-group">
                <label>ที่อยู่นิติบุคคล<span style="color:red">*</span></label>
                <input type="text" class="form-control" id="VillageAddress" value="{{$getVillageById->VillageAddress}}">
            </div>
            <div class="form-group">
                <label>หมู่ที่</label>
                <input type="text" class="form-control" id="VillageMoo" value="{{$getVillageById->VillageMoo}}">
            </div>
            <div class="form-group">
                <label>จังหวัด</label>
                <select name="VillageProvinceId" id="VillageProvinceId" data-id="VillageDistrictId">
                    @foreach($getProvince as $value)
                    <option value="{{$value->id}}" {{$value->id == $getVillageById->VillageProvinceId ? 'selected' : ''}}>
                        {{$value->name_th}}
                    </option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select name="VillageDistrictId" id="VillageDistrictId" data-id="VillageSubDistrictId">
                    @foreach($getAmphures as $value)
                    <option value="{{$value->id}}" {{$value->id == $getVillageById->VillageDistrictId ? 'selected' : ''}}>
                        {{$value->name_th}}
                    </option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select name="VillageSubDistrictId" id="VillageSubDistrictId" data-id="VillagePostCode">
                    @foreach($getTambons as $value)
                    <option value="{{$value->id}}" {{$value->id == $getVillageById->VillageSubDistrictId ? 'selected' : ''}}>
                        {{$value->name_th}}
                    </option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control" name="VillagePostCode" id="VillagePostCode" disabled value="{{$getVillageById->VillagePostCode}}">
            </div>
            <input type="hidden" value="{{$getVillageById->TransactionReqId}}" id="TransactionRequestId">
        </div>
...
<script>
    $(function() {
        ...
        $("form[name=editVillage]").submit(function(event) {
            ...
            //     return;
            // }
            var TransactionRequestId = $('#TransactionRequestId').val();
            ...
            formData.append('TransactionRequestId', TransactionRequestId);
            ...
            Swal.fire({
                title: "ยืนยัน",
                text: "คุณต้องการแก้ไขยื่นคำขอขึ้นทะเบียนใช่หรือไม่?",
                icon: "question",
