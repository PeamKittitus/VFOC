<?php namespace App\Http\Controllers;
	...
	class AdminUserController extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    public function cbInit() {
			...
			# OLD START FORM
			//$this->form = [];
			//$this->form[] = ["label"=>"IdCard","name"=>"idCard","type"=>"text","required"=>TRUE,"validation"=>"required|min:1|max:255"];
			...
			//$this->form[] = ["label"=>"Email","name"=>"email","type"=>"email","required"=>TRUE,"validation"=>"required|min:1|max:255|email|unique:user","placeholder"=>"Please enter a valid email address"];
		...
		{
			$getDataUser = DB::table('user')
			->leftjoin('systemOrgStructureProvince', 'systemOrgStructureProvince.id', 'user.orgProvinceId')
			->leftjoin('cms_users', 'cms_users.id', 'user.cmsUserId')
			...
			->select(
				...
				'user.email',
				'user.phoneNumber',
				'systemOrgStructureProvince.orgId',
				'cms_users.name',
			)
			->first();
