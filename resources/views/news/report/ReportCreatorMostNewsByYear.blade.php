...
<body>
    ...
    <ol class="breadcrumb page-breadcrumb">
        ...
        <li class="breadcrumb-item active">รายงานสรุปผู้ที่สร้างรายการข่าวสารมากที่สุด แยกรายปี</li>
    </ol>
    <div class="row">
        <div class="col-lg-12 sortable-grid ui-sortable" style="padding: 10px;">
            <div class="panel panel-sortable" role="widget">
                <div class="panel-hdr" role="heading">
                    <h2 class="ui-sortable-handle"><i class="fa fa-users" aria-hidden="true" style="margin-right: 15px"></i>รายงานสรุปผู้ที่สร้างรายการข่าวสารมากที่สุด แยกรายปี</h2>
                </div>
                                    ...
                                    <div class="col-sm-12">
                                        <table id="example" class="table table-striped table-bordered" cellspacing="0" width="100%">
                                            ...
                                            <tbody>
                                                ...
                                                $No = 1;
                                                @endphp
                                                @foreach ($ReportCreatorMostNews as $index => $rp)
                                                    <tr>
                                                        <td class="text-center">{{ $No++ }}.</td>
                                                        <td class="text-center">{{ $rp->TransactionYear }}</td>
                                                        <td class="text-center">{{ $rp->fullName }}</td>
                                                        <td class="text-center">{{ $rp->AmountMember }}</td>
                                                        <td class="text-center">{{ $rp->AmountPublic }}</td> 
                                                        <td class="text-center">{{ $rp->AmountMember + $rp->AmountPublic }}</td> 
                                                        <td class="text-center">
                                                            @if ($rp->AmountNews > 0)
                                                                {{ number_format(($rp->AmountNews * 100 / ( $rp->AmountMember + $rp->AmountPublic)), 2) }}
                                                            @else
                                                                0%
                                                            @endif
                                                        </td>                                                   
